...
${ogAlternates}
  ...
  <body data-language="${code}">
    ...
    <main id="main">
      ...
      <section class="section shell how" id="how-it-works"><div class="section-heading centered reveal"><div class="eyebrow"><span></span> ${h(t.how[0])}</div><h2>${h(t.how[1])}</h2></div><ol class="steps">${t.steps.map(([title, text], index) => `<li class="reveal"${index ? ` data-delay="${index}"` : ''}><span class="step-number">0${index + 1}</span><div class="step-icon">${['↓', 'A', '⌃⌥T'][index]}</div><h3>${h(title)}</h3><p>${h(text)}</p></li>`).join('')}</ol></section>
